...
- 애자일 방법론 중 하나인 eXtream Programming기반 설계
...
- **JUnit5를 이용하여 구성한다.**
  - **AssertEquals(실패시 반환 메시지, 예상값, 입력값) - 문자열 같은지 비교**
  - AssertArrayEquals(예상값, 입력값) - 배열 같은지 비교
  - AssertTrue(메시지, 논리값), AssertFalse(메시지, 논리값) - 논리 비교
  - AssertSame, AssertNotSame은 객체 같은지 비교
  - **스토리지가 필요한 경우 미리 map을 통해 스토리지 만든 후 JUnit Jupiter의 @AfterEach를 통해 테스트 후 비운다.**
  - **Given, when, then으로 나눈다. given은 주어진 데이터, when은 비즈니스로직, then은 비교 assert 이다.**
  - 시나리오 분석 후 단위 테스트를 나눠서 테스트케이스를 작성한 후 팀장님께 보고드린다.
  - 이때 데이터는 실제 차량 주행 후 나온 더미 데이터를 이용한다.
...
- **커서위치 삭제(구조체에 old parent, new parent로 undo redo 하기 때문), 사이클(부모 관계, google drive)**
...
- Lamport Timestamp Rule
  ...
  - **메시지가 도착하면 받은 clock counter와 자신이 가지고 있던 clock counter를 비교하여 자신이 가진 것이 크다면 그대로 유지하고, 새로 도착한 것이 더 크다면 새로운 clock counter에 1을 증가시킨 것을 자신의 clock counter로 셋팅한다.**
